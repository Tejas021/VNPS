...
useEffect(() => {
    const getPoliceComplain=async()=>{
        const data=await fetch('http://localhost:8000/api/policecomplain').then(response=>response.json())
        setpolicecomplains(data)
    }
    const getFireComplain=async()=>{
        const data=await fetch('http://localhost:8000/api/firecomplain').then(response=>response.json())
        setfirecomplains(data)
    }
    const getMedicalComplain=async()=>{
        const data=await fetch('http://localhost:8000/api/medicalcomplain').then(response=>response.json())
...
const addMedicalComplain=async(complain)=>{
    ...
    return (
      ...
      <h1>Sup</h1>
            ...
            <div className='  text-start'><button className='m-2 btn btn-outline-info  float-right' onClick={e=>{setdislpayPolice(!dislpayPolice);setdislpayMedical(false);setdislpayFire(false)}}>Police Emergency</button>
            <button className='m-2 btn btn-outline-danger  float-right' onClick={e=>{setdislpayMedical(!dislpayMedical);setdislpayPolice(false);setdislpayFire(false)}}>Medical Emergency</button>
            <button className=' m-2 btn btn-outline-warning  float-right' onClick={e=>{setdislpayFire(!dislpayFire);setdislpayMedical(false);setdislpayPolice(false)}}>Fire Emergency</button>
            </div>
